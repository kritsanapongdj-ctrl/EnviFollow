...
export default function App() {
      ...
      if (GOOGLE_SHEETS_WEBHOOK_URL && GOOGLE_SHEETS_WEBHOOK_URL !== "YOUR_GOOGLE_APPS_SCRIPT_WEB_APP_URL") {
        ...
        try {
          await fetch(GOOGLE_SHEETS_WEBHOOK_URL, {
            ...
            body: JSON.stringify({
              ...formData,
              status: isPassed ? "ผ่านเกณฑ์" : "ไม่ผ่านเกณฑ์",
              system_link: window.location.href
            })
          });
        } catch (fetchErr) {
          console.error("Error syncing to Webhook:", fetchErr);
        }
      }
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col md:flex-row font-sans overflow-x-hidden">
      ...
      {/* Sidebar */}
      <aside className={`
        fixed inset-y-0 left-0 z-40 w-72 bg-[#002D62] text-white flex flex-col shadow-xl transition-transform duration-300 transform 
...
function NavItem({ icon, label, active, onClick, isLocked }) {
  return (
    <button 
      onClick={onClick}
      className={`w-full flex items-center justify-between gap-4 px-5 py-3 rounded-xl transition-all ${active ? 'bg-blue-700 text-white shadow-lg ring-1 ring-blue-500/50' : 'text-blue-100 hover:bg-blue-800/50'}`}
    >
    ...
    <div className="space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex bg-white rounded-lg shadow-sm border p-1 w-full sm:w-auto">
          {[3, 6].map(m => (
            <button key={m} onClick={() => setPeriod(m)} className={`flex-1 sm:flex-none px-6 py-2 rounded-md text-sm font-bold ${period === m ? 'bg-[#002D62] text-white' : 'text-gray-500'}`}>
              {m} เดือน
            </button>
          ))}
        </div>
...
function StatCard({ title, value, total, isAlert, icon, color, description }) {
  return (
...
function ChartBox({ title, data, dataKey, limits, color, yDomain }) {
  return (
    <div className="bg-white p-6 md:p-8 rounded-2xl shadow-sm border border-gray-100">
      <h3 className="font-bold text-gray-700 mb-6 flex items-center gap-2 text-sm sm:text-base">
        ...
        <ResponsiveContainer width="100%" height="100%">
          <LineChart data={data}>
            ...
            {limits.map((limit, idx) => (
              <ReferenceLine key={idx} y={limit} stroke="#ef4444" strokeDasharray="5 5" label={{ position: 'right', value: `เกณฑ์ ${limit}`, fill: '#ef4444', fontSize: 10, fontWeight: 'bold' }} />
            ))}
...
function ReportView({ logs }) {
  ...
  const exportToCSV = () => {
    ...
    const rows = filteredLogs.map(l => {
      ...
      return [l.date, l.location, l.ph, l.tds, l.color, l.odor, l.recorder, isPassed ? "ผ่าน" : "ไม่ผ่าน"];
    });
    ...
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white p-6 rounded-2xl shadow-sm border border-gray-100 print:hidden">
        <div>
          ...
          <p className="text-sm text-gray-500">กรองข้อมูลรายเดือนเพื่อตรวจสอบและส่งออก</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 print:hidden">
        <button onClick={handlePrint} className="flex items-center justify-center gap-3 bg-[#002D62] text-white p-4 rounded-2xl font-bold shadow-lg hover:bg-[#003d82] transition-all">
          <Printer size={20} /> พิมพ์รายงาน (PDF)
        </button>
        <button onClick={exportToCSV} className="flex items-center justify-center gap-3 bg-[#B8904F] text-white p-4 rounded-2xl font-bold shadow-lg hover:bg-[#a67d3e] transition-all">
          <Download size={20} /> ส่งออกข้อมูล (CSV)
        </button>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-50 print:divide-gray-200">
              {filteredLogs.length === 0 ? (
                <tr>
                  <td colSpan="5" className="p-10 text-center text-gray-400 font-medium italic">ไม่มีข้อมูลการบันทึกในเดือนนี้</td>
                </tr>
              ) : (
                filteredLogs.map((l, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-blue-50/30 transition-colors print:hover:bg-transparent">
                      ...
                      <td className="p-4 text-sm text-gray-600 print:text-black">{l.location}</td>
...
function FormField({ label, children, alert }) {
  return (
    <div className="space-y-2">
      ...
      {children}
      {alert && <p className="text-red-500 text-[10px] font-bold flex items-center gap-1.5 animate-pulse italic"><AlertCircle size={12}/> {alert}</p>}
    </div>
  );
}
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6 md:space-y-8 animate-in fade-in duration-500">
      <div className="bg-white p-8 md:p-10 rounded-3xl shadow-xl border border-gray-100">
        <h2 className="text-xl md:text-2xl font-bold text-[#002D62] mb-8 flex items-center gap-3">
          <User size={32} className="text-[#B8904F]" /> จัดการรายชื่อเจ้าหน้าที่
        </h2>
        <div className="flex flex-col sm:flex-row gap-3 mb-8">
          <input value={name} onChange={e => setName(e.target.value)} placeholder="ระบุชื่อ-นามสกุล..." className="flex-1 p-4 rounded-xl border-gray-200 bg-slate-50 border focus:ring-2 focus:ring-[#002D62] outline-none font-medium" />
          <button 
            onClick={() => { if(name.trim()) { onUpdateStaff([...settings.staffNames, name.trim()]); setName(''); }}}
            className="bg-[#B8904F] text-white px-8 py-4 sm:py-0 rounded-xl font-black hover:shadow-lg transition-all active:scale-95"
          >เพิ่มรายชื่อ</button>
        </div>
