...
// ----------------------------------------------------------------------
// 🔥 ตั้งค่าระบบของคุณ (ใส่ลิงก์ของคุณที่ 2 บรรทัดด้านล่างนี้)
// ----------------------------------------------------------------------

// 1. URL จาก Google Apps Script (Webhook)
...
// 2. URL สำหรับปุ่ม "เปิดชีต" (ก๊อปปี้ลิงก์ Google Sheets ของคุณมาวางเลยครับ)
...
export default function App() {
  ...
  const fetchLogsFromSheets = async () => {
    try {
      ...
      try {
        ...
        throw new Error("ระบบถูกบล็อก: โปรดตรวจสอบ URL ของ Webhook หรือสิทธิ์การเข้าถึงใน Apps Script");
      }
  ...
  const updateStaff = async (newStaffData) => {
    setSettings({ staffData: newStaffData }); 
    localStorage.setItem('waterQC_StaffCache', JSON.stringify({ staffData: newStaffData }));
    try {
      await fetch(GOOGLE_SHEETS_WEBHOOK_URL, {
        method: 'POST',
        mode: 'no-cors',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ action: 'update_settings', data: { staffData: newStaffData } })
      });
    } catch (err) {
      console.error(err);
    }
...
function EntryForm({ onSubmit, staffData }) {
  ...
  useEffect(() => {
    if (staffData && staffData.length > 0 && !formData.recorder) {
      ...
      setFormData(p => ({ 
        ...p, 
        recorder: firstStaff.name,
        location: firstStaff.projects && firstStaff.projects.length > 0 ? firstStaff.projects[0] : ''
      }));
    }
  }, [staffData]);
  ...
  const handleChange = (e) => {
    ...
    if (name === 'recorder') {
      ...
      setFormData(p => ({ ...p, recorder: value, location: newLocation }));
    } else {
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[110] p-4">
          <div className="bg-white rounded-3xl p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 duration-200 border-t-8 border-red-500">
            ...
            <div className="flex gap-4">
              ...
              <button type="button" onClick={() => { setShowConfirmModal(false); onSubmit(formData); }} className="flex-1 py-3.5 rounded-xl font-bold text-white bg-red-600 hover:bg-red-700 shadow-lg transition-all">ยืนยันบันทึก</button>
            </div>
          </div>
        </div>
      )}
